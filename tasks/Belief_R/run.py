...
sys.path.insert(0, str(project_root / "tasks"))

from src import runner
from tasks.Belief_R.prompts import get_template, build_prompt
from tasks.Belief_R.metrics import compute_metrics, normalize_pred, get_gold_label
...
def extract_gold_answers(data):
    """提取标准答案（a/b/c）"""
    return [get_gold_label(row) for row in data]
...
def main():
    ...
    for i in range(experiment_config["repeats"]):
        ...
        predictions = [normalize_pred(getattr(r, "answer", "")) for r in repeat_results]
    ...
    runner.save_common_results(
        ...
        is_correct_fn=lambda pred, gold: normalize_pred(pred) == gold,
    )
